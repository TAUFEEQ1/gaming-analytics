...
class TrainHybridModels(luigi.Task):
    ...
    def _assign_tiers(self, df):
        """Assign operators to tiers using DBSCAN on movement and payout flow features"""
        operator_stats = df.groupby('operator').agg({
            'movement_wager_amt': 'mean',
            'movement_win_amt': 'mean',
            'payout_base_win': 'mean',
            'revenue_amt': 'mean'
        }).reset_index()
        
        operator_stats.columns = ['operator', 'movement_wager_mean', 'movement_win_mean', 'payout_base_win_mean', 'revenue_amt_mean']
        
        # Separate zero and non-zero based on key payout flow variables
        zero_ops = operator_stats[
            (operator_stats['movement_wager_mean'] == 0) | 
            (operator_stats['movement_win_mean'] == 0) |
            (operator_stats['payout_base_win_mean'] == 0)
        ]['operator'].tolist()
        ...
        # Cluster non-zero operators using multiple payout flow indicators
        if len(non_zero_ops) > 0:
            ...
            non_zero_ops['log_payout'] = np.log10(non_zero_ops['payout_base_win_mean'])
            non_zero_ops['log_revenue'] = np.log10(non_zero_ops['revenue_amt_mean'].abs() + 1)  # Add 1 for negative values
            
            clustering_features = non_zero_ops[['log_wager', 'log_win', 'log_payout', 'log_revenue']].values
    ...
    def _evaluate_operators(self, df):
        ...
        feature_cols = [col for col in df_encoded.columns 
                       if col not in ['payout_base_win', 'tier', 'timestamp_end']]
